...
contract BasicDutchAuction {
  ...
  function bid() external payable returns (address) {
    ...
    //if the gwei value sent is less than the current accepted price
    require(msg.value >= currentPrice,"The wei value sent is not acceptable");

    //Since there is no actual asset to transfer on-chain,we set the bidder's address as winner and stop the auction
